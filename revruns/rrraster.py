...
def h5(file, dst, dataset, res, crs, agg_fun, layer, fltr, mask):
    ...
    rasterize(gdf, res, dst, mask)


def gpkg(file, dst, dataset, res, crs, mask):
...
def rasterize(gdf, res, dst, mask):
...
def main(src, dst, dataset, resolution, crs, agg_fun, layer, filter, mask):
    """
    file = "/shared-projects/rev/projects/soco/data/points/mean_ws/wtk_means_se_2007_2013.gpkg"
    dst = "/shared-projects/rev/projects/soco/rev/runs/reference/results/rasters/wtk_mean_120m.tif"
    dataset = "windspeed_120m"
    res = 2000
    crs = "EPSG:3466"
    agg_fun = "mean"
    layer = None
    ...
    if extension == ".h5":
        h5(src, dst, dataset, resolution, crs, agg_fun, layer, filter, mask)

    # elif extension == ".csv":
        # csv(src, dst, dataset, resoltuion, crs, mask)

    elif extension == ".gpkg":
        gpkg(src, dst, dataset, resolution, crs, mask)
