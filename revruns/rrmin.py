...
# The command
@click.command()
@click.option("-file", help=file_help)
@click.option("-dataset", help=dataset_help)
def main(file, dataset):
    """Prints the minimum value of an hdf5 file data set."""
    # Open the data set
    ...
    print("Calculating min value for {}:{} using {} partitions".format(file, dataset, npart))

    # And compute the min value
    with pb():
        result = dds.min().compute()
